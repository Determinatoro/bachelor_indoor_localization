...
test_paths = glob.glob(f'{BASE_PATH}/input/indoor-location-navigation/train/*/*/*')
...
for path in test_paths:
    ...
    for line_data in lines:
        ...
        if is_site[0] == "SiteID":
            site = list(filter(lambda x: x.site_id == site_id, site_list))
            if len(site) == 0:
                break
...
for site in site_list:
    for BSSID in site.BSSIDs:
        if site.BSSIDs[BSSID] >= 1000:
            count += 1

print(f"Number of BSSIDs that has been observed over a 1000 times: {count}")
