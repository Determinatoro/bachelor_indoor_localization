...
def create_model(input_data):
    ...
    x = L.Reshape((1, -1))(x)
    x = L.BatchNormalization()(x)
    x = L.Bidirectional(L.LSTM(256, dropout=0.3, recurrent_dropout=0.3, return_sequences=True, activation='relu'))(x)
...
for fold, (trn_idx, val_idx) in enumerate(
        StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED).split(data.loc[:, 'path'],
                                                                                  data.loc[:, 'path'])):
    # Train set
    x_train_set = data.loc[trn_idx, BSSID_FEATS + RSSI_FEATS + ['floor', 'site_id']]
    ...
    # Validation set
    x_val_set = data.loc[val_idx, BSSID_FEATS + RSSI_FEATS + ['floor', 'site_id']]
    ...
    history = model.fit(
        ...
        y_train,
        validation_data=(
        [x_val_set.loc[:, BSSID_FEATS], x_val_set.loc[:, RSSI_FEATS + ['floor']], x_val_set.loc[:, 'site_id']],
        y_valid),
    ...
    # Make predictions
    pred = model.predict([test_data.loc[:, BSSID_FEATS], test_data.loc[:, RSSI_FEATS + ['floor']],
                          test_data.loc[:, 'site_id']])
    preds_x += pred[:, 0]
    preds_y += pred[:, 1]
...
# Save submission
all_preds.to_csv(f'submission_blstm_x_y_floor_{NUM_FEATS}_{timestamp}.csv')